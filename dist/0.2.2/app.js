...
export function createApp(context, doc = document) {
    ...
    function mountDrawer() {
        ...
        on(drawer, 'click', e => {
            ...
            if (action === 'open') open();
            if (action === 'npcs') { active = 'npc'; open(); }
    ...
    function mountDialog() {
        ...
        dialog.innerHTML = `<div class="vc-atmosphere" aria-hidden="true"><div class="vc-mist"></div><div class="vc-orbit"></div><div class="vc-motes">${Array.from({ length: 16 }, (_, i) => `<i style="--n:${i};left:${(i * 37) % 100}%;top:${(i * 23) % 100}%"></i>`).join('')}</div></div>
          ...
          <footer class="vc-footer"><span>${icon('gem')}<span>THE LIVING LEDGER · ${VERSION}</span></span><button type="button" data-command="drawer">${icon('spark')}<span class="vc-customize">Appearance & effects</span></button></footer>`;
